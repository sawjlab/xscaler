...
// Load data from RPC server (hall C style)
   Int_t LoadDataRPC(const char* server); 
...
// RATES (Hz) since last update, similar usage to above.
   ...
   Int_t GetClockSlot() { return clkslot; };
   Int_t GetClockChan() { return clkchan; };
   Double_t GetClockRate() { return clockrate; }; 
