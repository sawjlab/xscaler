     ...
     */
{
  ...
  for(i=0;i<nchan;i++) {
    temp = scalerBuffer[i];
    increment = l1877scaler->nhitslist[i]*ratio;
...
const MAX_1877_DATA = 1600;
const SFI_VME_ADDR = 0xe00000;
...
void L1877ReadoutTask() {
  ...
  */
  ...
  int slot, lastslot, chan, header, itdc, nwrds;
  unsigned long rb;
  int scalchan;
  int L1877_Window=4488;
  int L1877_MinSlot=14;		/* Slots 14, 15, 16 */
  int res;
  unsigned int laddr,dmaptr;
  ...
  if((res=sysLocalToBusAdrs(0x09,l1877data,&dmaptr)) !=0) {
    printf("Error Initializing SFI res=%d\n",res);
    goto fooy;
  } else {
...
/****** Read TDC Module ID *********/
  ...
  while(!scaler_readout_killer_flag) {
    ...
    taskDelay(60);		/* Wait for  conversion */
    res=fprcm(9,0,&datascan);
    /*printf("Datascan=%8.8x\n",datascan);*/
    for(i=0;i<num_1877modules;i++) {
      slot = L1877_MinSlot+i;
      fpwc(slot,0,0x400);	/* Load next event */
      sfi_error_decode(0);
    }
    if((datascan&L1877_SCAN_MASK) == L1877_SCAN_MASK) {
      slot = L1877_MinSlot+num_1877modules-1;
      rb = 0;
      res=fb_frdb_1(slot,0,dmaptr,4*MAX_1877_DATA,&rb,1,0,1,0,0x0a,0,0,1);
      sfi_error_decode(0);
      nwrds = rb/4;
      if(rb > (4*MAX_1877_DATA+4) || res != 0) {
	printf("fpbr error pa=%d, res=0x%x maxBytes=%d retBytes=%d fifo=0x%x\n",
	       slot,res,4*MAX_1877_DATA,rb,sfiSeq2VmeFifoVal);
	...
	for(ii=0;ii<nwrds;ii++) {
	  ...
	  if(!header) {
	    ...
	    if(itdc>=0&&itdc<num_1877modules) {
	      chan = (dataword>>17)&0x7f;
	      scalchan = itdc*96+chan;
	      l1877scaler.nhitslist[scalchan]++;
	    }
	  } else {
	    header = 0;
	  }
