...
22.02.2006 (saw) Include F1 errors.
...
/*  printf("Sum = %d, time=%f, ratio=%f\n",f1scaler->window_sum,timedif,ratio);*/
  ...
  for(i=0;i<nchan;i++) {
    temp = scalerBuffer[i];
    increment = f1scaler->nhitslist[i]*ratio;
    scalerBuffer[i] += increment;
    if(scalerBuffer[i] < temp) scalerOverflows[i]++;
    f1scaler->nhitslist[i]=0;
  }
  i = num_f1modules*F1SIZE;
  nchan = num_f1modules*8*5;
  if(f1scaler->errorlist_size < nchan) nchan = f1scaler->errorlist_size;
  for(j=0;j<nchan;j++) {
    temp = scalerBuffer[i+j];
    scalerBuffer[i+j] += f1scaler->nerrorslist[j];
    if(scalerBuffer[i+j] < temp) scalerOverflows[i+j];
    f1scaler->nerrorslist[j]=0;
  }
  i = num_f1modules*F1SIZE + num_f1modules*8*5;
...
void setup_scalers(int nf1,int between_runs)
{
  ...
  scalerBufferSize = F1SIZE*num_f1modules+num_f1modules*8*5+2;
...
#define F1_HIT_OFLW (1<<24)
#define F1_OUT_OFLW (1<<25)
#define F1_RES_LOCK (1<<26)
#define F1_RESP_FAIL (1<<22)
#define F1_TRGF_OFLW (1<<22)
#define F1_ERR_MASK (F1_HIT_OFLW&F1_OUT_OFLW&F1_RES_LOCK)
#define F1_NOERR F1_RES_LOCK
...
void f1ReadoutTask() {
  ...
  */
  ...
  unsigned int stat;
  static struct F1SCALER f1scaler = {0,0,0,0,0,0,0,0,0};
  int slotmap[32];
  unsigned int dataword;
  int slot, if1, nwrds, chan, chip, notheadertrailer;
  int f1_Window=500;
  ...
  f1scaler.errorlist_size = nf1tdc*8*5; /* 5 error types/chip 8chips/slot */
  f1scaler.nerrorslist = (int *) malloc(f1scaler.errorlist_size*sizeof(int));
  for(i=0;i<f1scaler.errorlist_size;i++) {
    f1scaler.nerrorslist[i] = 0;
  }
  ...
  f1GSetWindow(f1_Window,600,CHIP_MASK);
  f1GClear();
  ...
  while(!scaler_readout_killer_flag) {
	...
	for(ii=0;ii<nwrds;ii++) {
	  ...
	  /* Check for Resunlock, hit fifo overflow, and output fifo overflow */
	  if(if1>=0 && if1<nf1tdc) {
	    if(notheadertrailer) {
	      chip = (dataword&0x380000) >> 19;
	    } else {
	      chip = (dataword&0x38) >> 3;
	    }
	    if((dataword&F1_ERR_MASK) != F1_NOERR) {
	      if(!(dataword&F1_RES_LOCK)) {
		f1scaler.nerrorslist[if1*8*5+chip*5]++;
	      }
	      if((dataword&F1_OUT_OFLW)) {
		f1scaler.nerrorslist[if1*8*5+chip*5+1]++;
	      }
	      if((dataword&F1_HIT_OFLW)) {
		f1scaler.nerrorslist[if1*8*5+chip*5+2]++;
	      }
	    }
	    if(notheadertrailer) {
	      if(dataword&F1_RESP_FAIL) {
		f1scaler.nerrorslist[if1*8*5+chip*5+3]++;
	      }
	      chan = (dataword>>16)&0x3f;
	      f1scaler.nhitslist[if1*64+chan]++;
	    } else {
	      if(dataword&F1_TRGF_OFLW) {
		f1scaler.nerrorslist[if1*8*5+chip*5+4]++;
	      }
	    }
	  }
	  /*	  printf("%d %d 0x%8.8x\n",if1,chan,dataword);*/
