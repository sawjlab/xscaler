...
INCLUDES     = $(ROOTCFLAGS) -I$(DCDIR) scaserCalls.h
...
# all this needed for ET...  up to comment below
   ...
   ALL_LIBS =  $(ROOTGLIBS) $(LIBDC) $(LIBRPC)
ifndef STANDALONE
...
PROGS = tscalfile tscalasy tscalhist tscalonl 
PROGS += tscalntup tscaldtime tscalroc11 tscalevt tscalring
PROGS += xscaler
...
tscalonl: tscalonl_main.o  $(SCALER_OBJS) $(SRC) $(HEAD) $(LIBDC)  $(LIBRPC)
	rm -f $@
	$(CXX) $(CXXFLAGS) -o $@ tscalonl_main.o  $(SCALER_OBJS) $(ALL_LIBS) 

xscaler: tscalgui_main.o $(SCALER_OBJS) $(SRC) $(HEAD) $(LIBDC)  $(LIBRPC) 
	rm -f $@
...
#	$(ROOTSYS)/bin/rootcint -f THaScalDict.C -c $(INCLUDES) $^
...
install:	all
	...
	cp -puvf $(SRC) $(HEAD) haScal_LinkDef.h Makefile $(ANALYZER)/src/hana_scaler
	cp -puvf $(HEAD) $(ANALYZER)/include
