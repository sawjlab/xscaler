...
//   THaScaler
//
//   Scaler Data in Hall A/C at Jlab.  
...
Int_t THaScaler::LoadDataRPC(const char* host) {

// RPC method used by Hall C.  Jan 2017
// This assumes 32 channels per scaler.  and a clock at 60 Hz.
// It will break otherwise.  (I need to fix that.)
// However, we do at least check for consistency, see below.
...
// but use the values found here anyway.
	...
	rpchandle=0;		// Let it try to reopen next time
     ...
     if (ldebug) { // Steve Wood's printout
	  ...
	  if (ldebug)  printf("%d: %u(%u)\n",i,rpcscalers[i],rpcoverflows[i]);
	}
     }

     ntot=0;
     for (Int_t jslot = 0; jslot < SCAL_NUMBANK; jslot++) {
        Int_t nchan = chanPerScaler;
        for (Int_t k = 0; k < nchan; k++) {
           Int_t i = jslot*SCAL_NUMCHAN + k;
             if (i < SCAL_NUMBANK*SCAL_NUMCHAN && ntot < rpcchannels) {
	         rawdata[i] = rpcscalers[ntot++];
             } 
	}
     }
...
// This is hall C, so we will assume we always use RPC.
#ifdef HALLC_ALWAYS
  return LoadDataRPC(vme_server.c_str());
#endif
#ifdef HALLA_ALWAYS
