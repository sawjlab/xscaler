...
TROOT root("Scalers","Hall C Scaler GUI", initfuncs);
void usage();
...
int main(int argc, char **argv) {
   ...
   if (argc > 1) {
     bank = argv[1];
     if (bank != "Left" && bank != "LEFT" && bank != "Right" && bank != "RIGHT" && bank != "SHMS" ) {
         usage();
         return 1;
     }
   } else {
     bank = "SHMS";       // SHMS is default
...
void usage() {
  ...
  cout << "where bank = `SHMS' or something"<<endl;
  cout << "(without the quotes)"<<endl;
  cout << "and default bank is `SHMS' (i.e. if no arg)"<<endl;
}
